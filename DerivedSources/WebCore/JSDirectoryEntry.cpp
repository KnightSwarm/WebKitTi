...
bool JSDirectoryEntryPrototype::getOwnPropertySlot(JSObject* object, ExecState* exec, PropertyName propertyName, PropertySlot& slot)
{
...
EncodedJSValue JSC_HOST_CALL jsDirectoryEntryPrototypeFunctionCreateReader(ExecState* exec)
{
    ...
    if (!thisValue.inherits(JSDirectoryEntry::info()))
        return throwVMTypeError(exec);
    JSDirectoryEntry* castedThis = jsCast<JSDirectoryEntry*>(asObject(thisValue));
    ASSERT_GC_OBJECT_INHERITS(castedThis, JSDirectoryEntry::info());
...
{
    return value.inherits(JSDirectoryEntry::info()) ? jsCast<JSDirectoryEntry*>(asObject(value))->impl() : 0;
}
