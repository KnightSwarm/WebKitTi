...
bool JSEntryPrototype::getOwnPropertySlot(JSObject* object, ExecState* exec, PropertyName propertyName, PropertySlot& slot)
{
...
bool JSEntry::getOwnPropertySlot(JSObject* object, ExecState* exec, PropertyName propertyName, PropertySlot& slot)
{
...
EncodedJSValue JSC_HOST_CALL jsEntryPrototypeFunctionGetMetadata(ExecState* exec)
{
    ...
    if (!thisValue.inherits(JSEntry::info()))
        return throwVMTypeError(exec);
    JSEntry* castedThis = jsCast<JSEntry*>(asObject(thisValue));
    ASSERT_GC_OBJECT_INHERITS(castedThis, JSEntry::info());
    Entry* impl = static_cast<Entry*>(castedThis->impl());
...
{
    return value.inherits(JSEntry::info()) ? jsCast<JSEntry*>(asObject(value))->impl() : 0;
}
