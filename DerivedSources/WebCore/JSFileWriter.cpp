...
bool JSFileWriterPrototype::getOwnPropertySlot(JSObject* object, ExecState* exec, PropertyName propertyName, PropertySlot& slot)
{
...
bool JSFileWriter::getOwnPropertySlot(JSObject* object, ExecState* exec, PropertyName propertyName, PropertySlot& slot)
{
...
EncodedJSValue JSC_HOST_CALL jsFileWriterPrototypeFunctionWrite(ExecState* exec)
{
    ...
    if (!thisValue.inherits(JSFileWriter::info()))
        return throwVMTypeError(exec);
    JSFileWriter* castedThis = jsCast<JSFileWriter*>(asObject(thisValue));
    ASSERT_GC_OBJECT_INHERITS(castedThis, JSFileWriter::info());
    FileWriter* impl = static_cast<FileWriter*>(castedThis->impl());
...
{
    return value.inherits(JSFileWriter::info()) ? jsCast<JSFileWriter*>(asObject(value))->impl() : 0;
}
