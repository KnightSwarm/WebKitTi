...
bool JSPerformanceEntryListPrototype::getOwnPropertySlot(JSObject* object, ExecState* exec, PropertyName propertyName, PropertySlot& slot)
{
...
bool JSPerformanceEntryList::getOwnPropertySlot(JSObject* object, ExecState* exec, PropertyName propertyName, PropertySlot& slot)
{
...
bool JSPerformanceEntryList::getOwnPropertySlotByIndex(JSObject* object, ExecState* exec, unsigned index, PropertySlot& slot)
{
    JSPerformanceEntryList* thisObject = jsCast<JSPerformanceEntryList*>(object);
    ASSERT_GC_OBJECT_INHERITS(thisObject, info());
...
EncodedJSValue JSC_HOST_CALL jsPerformanceEntryListPrototypeFunctionItem(ExecState* exec)
{
    ...
    if (!thisValue.inherits(JSPerformanceEntryList::info()))
...
{
    return value.inherits(JSPerformanceEntryList::info()) ? jsCast<JSPerformanceEntryList*>(asObject(value))->impl() : 0;
}
