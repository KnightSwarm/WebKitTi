...
bool JSDirectoryEntrySyncPrototype::getOwnPropertySlot(JSObject* object, ExecState* exec, PropertyName propertyName, PropertySlot& slot)
{
...
EncodedJSValue JSC_HOST_CALL jsDirectoryEntrySyncPrototypeFunctionCreateReader(ExecState* exec)
{
    ...
    if (!thisValue.inherits(JSDirectoryEntrySync::info()))
        return throwVMTypeError(exec);
    JSDirectoryEntrySync* castedThis = jsCast<JSDirectoryEntrySync*>(asObject(thisValue));
    ASSERT_GC_OBJECT_INHERITS(castedThis, JSDirectoryEntrySync::info());
...
{
    return value.inherits(JSDirectoryEntrySync::info()) ? jsCast<JSDirectoryEntrySync*>(asObject(value))->impl() : 0;
}
