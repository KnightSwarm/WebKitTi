...
bool JSDirectoryReaderSyncPrototype::getOwnPropertySlot(JSObject* object, ExecState* exec, PropertyName propertyName, PropertySlot& slot)
{
...
EncodedJSValue JSC_HOST_CALL jsDirectoryReaderSyncPrototypeFunctionReadEntries(ExecState* exec)
{
    ...
    if (!thisValue.inherits(JSDirectoryReaderSync::info()))
        return throwVMTypeError(exec);
    JSDirectoryReaderSync* castedThis = jsCast<JSDirectoryReaderSync*>(asObject(thisValue));
    ASSERT_GC_OBJECT_INHERITS(castedThis, JSDirectoryReaderSync::info());
...
{
    return value.inherits(JSDirectoryReaderSync::info()) ? jsCast<JSDirectoryReaderSync*>(asObject(value))->impl() : 0;
}
