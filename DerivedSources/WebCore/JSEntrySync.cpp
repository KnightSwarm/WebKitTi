...
bool JSEntrySyncPrototype::getOwnPropertySlot(JSObject* object, ExecState* exec, PropertyName propertyName, PropertySlot& slot)
{
...
bool JSEntrySync::getOwnPropertySlot(JSObject* object, ExecState* exec, PropertyName propertyName, PropertySlot& slot)
{
...
EncodedJSValue JSC_HOST_CALL jsEntrySyncPrototypeFunctionGetMetadata(ExecState* exec)
{
    ...
    if (!thisValue.inherits(JSEntrySync::info()))
        return throwVMTypeError(exec);
    JSEntrySync* castedThis = jsCast<JSEntrySync*>(asObject(thisValue));
    ASSERT_GC_OBJECT_INHERITS(castedThis, JSEntrySync::info());
    EntrySync* impl = static_cast<EntrySync*>(castedThis->impl());
    ExceptionCode ec = 0;
...
{
    return value.inherits(JSEntrySync::info()) ? jsCast<JSEntrySync*>(asObject(value))->impl() : 0;
}
