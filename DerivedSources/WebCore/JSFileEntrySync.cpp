...
bool JSFileEntrySyncPrototype::getOwnPropertySlot(JSObject* object, ExecState* exec, PropertyName propertyName, PropertySlot& slot)
{
...
EncodedJSValue JSC_HOST_CALL jsFileEntrySyncPrototypeFunctionFile(ExecState* exec)
{
    ...
    if (!thisValue.inherits(JSFileEntrySync::info()))
        return throwVMTypeError(exec);
    JSFileEntrySync* castedThis = jsCast<JSFileEntrySync*>(asObject(thisValue));
    ASSERT_GC_OBJECT_INHERITS(castedThis, JSFileEntrySync::info());
...
{
    return value.inherits(JSFileEntrySync::info()) ? jsCast<JSFileEntrySync*>(asObject(value))->impl() : 0;
}
