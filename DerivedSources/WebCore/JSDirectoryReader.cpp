...
bool JSDirectoryReaderPrototype::getOwnPropertySlot(JSObject* object, ExecState* exec, PropertyName propertyName, PropertySlot& slot)
{
...
EncodedJSValue JSC_HOST_CALL jsDirectoryReaderPrototypeFunctionReadEntries(ExecState* exec)
{
    ...
    if (!thisValue.inherits(JSDirectoryReader::info()))
        return throwVMTypeError(exec);
    JSDirectoryReader* castedThis = jsCast<JSDirectoryReader*>(asObject(thisValue));
    ASSERT_GC_OBJECT_INHERITS(castedThis, JSDirectoryReader::info());
...
{
    return value.inherits(JSDirectoryReader::info()) ? jsCast<JSDirectoryReader*>(asObject(value))->impl() : 0;
}
