...
bool JSFileEntryPrototype::getOwnPropertySlot(JSObject* object, ExecState* exec, PropertyName propertyName, PropertySlot& slot)
{
...
EncodedJSValue JSC_HOST_CALL jsFileEntryPrototypeFunctionCreateWriter(ExecState* exec)
{
    ...
    if (!thisValue.inherits(JSFileEntry::info()))
        return throwVMTypeError(exec);
    JSFileEntry* castedThis = jsCast<JSFileEntry*>(asObject(thisValue));
    ASSERT_GC_OBJECT_INHERITS(castedThis, JSFileEntry::info());
...
{
    return value.inherits(JSFileEntry::info()) ? jsCast<JSFileEntry*>(asObject(value))->impl() : 0;
}
